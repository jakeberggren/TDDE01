...
sum <- 0
while (sum < 95) {
  sum <- sum(sum(eig$values[1:i]))
  i <- i + 1
}
sum(eig$values[1:i - 1]) # 95.01883, i - 1 = 34
...
plot(multi_mse_train[500:8000],
     ylim = c(0.2, 0.8), ylab = "Mean square error", xlab = "# of iterations",
     pch = ".", xlim = c(500,7000))
...
legend("topright", c("test data", "training data"),
